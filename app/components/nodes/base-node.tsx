...
export const BaseNode = forwardRef<HTMLDivElement, BaseNodeProps>(({ className, status, ...props }, ref) => (
	<div
		ref={ref}
		className={cn(
			"relative rounded-md bg-card border border-muted-foreground text-card-foreground hover:bg-accent",
			status !== "initial" && "border-none",
			className,
		)}
		tabIndex={0}
		{...props}
	>
		...
		{status === "success" && (
			<CheckCircleIcon className="absolute right-0.5 bottom-0.5 size-2 text-emerald-600 stroke-3" />
		)}
		{status === "loading" && <Spinner className="absolute -right-0.5 -bottom-0.5 size-2 text-blue-700 stroke-3" />}
	</div>
))
