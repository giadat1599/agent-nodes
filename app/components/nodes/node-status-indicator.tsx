...
const StatusBorder = ({ children, className }: { children: ReactNode; className?: string }) => {
	return (
		<>
			<div
				className={cn(
					"absolute -left-[1px] -top-[1px] h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-[7px] border",
					className,
				)}
			/>
			{children}
...
export const NodeStatusIndicator = ({ status, variant = "border", children, className }: NodeStatusIndicatorProps) => {
	switch (status) {
		...
		case "success":
			return <StatusBorder className={cn("border-emerald-600", className)}>{children}</StatusBorder>
		case "error":
